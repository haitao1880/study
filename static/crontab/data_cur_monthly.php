...
$premiketime = mktime(0,0,0,date("m"),1,date("Y"));
$curmiketime = mktime(23,59,59,date("m")+1,0,date("Y"));
...
if($dailyViewRow){
	...
	$bind = array();
	$bind = array(intval($viewRow['total_start_times']),intval($viewRow['total_pv']),$viewRow['per_view'],intval($viewRow['total_online_times']),$viewRow['per_online'],$month);
    ...
    $bind = array($month,intval($viewRow['total_start_times']),intval($viewRow['total_pv']),$viewRow['per_view'],intval($viewRow['total_online_times']),$viewRow['per_online']);
...
if($dailyMemberRow){
    $updateSql = "UPDATE rhc_member_monthly SET new_user = ?,old_user = ?,active_user = ?,open_num = ? WHERE month = ?";
	$bind = array();
	$bind = array(intval($userRow['total_new_user']),$old_user,intval($userTempRow['total_active_user']),intval($userRow['total_open_num']),$month);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_member_monthly(month,new_user,old_user,active_user,open_num) VALUES (?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($month,intval($userRow['total_new_user']),$old_user,intval($userTempRow['total_active_user']),intval($userRow['total_open_num']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
if($dailyPointRow){
	...
	$bind = array();
	$bind = array(intval($pointRow['total_send_point']),intval($pointRow['total_use_point']),intval($pointRow['sign_point']),intval($pointRow['download_point']),intval($pointRow['buy_point']),intval($pointRow['package_point']),intval($pointRow['huoban_point']),intval($pointRow['huafei_point']),intval($pointRow['other_point']),$month);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
	...
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($month,intval($pointRow['total_send_point']),intval($pointRow['total_use_point' ]),intval($pointRow['sign_point']),intval($pointRow['download_point']),intval($pointRow['buy_point']),intval($pointRow['package_point']),intval($pointRow['huoban_point']),intval($pointRow['huafei_point']),intval($pointRow['other_point']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
if($dailyClickRow){
	...
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($month,intval($clickRow['btn_201']),intval($clickRow['btn_202']),intval($clickRow['btn_203']),intval($clickRow['btn_204']),intval($clickRow['btn_205']),intval($clickRow['btn_207']),intval($clickRow['btn_208']),intval($clickRow['btn_301']),intval($clickRow['btn_302']),intval($clickRow['btn_303']),intval($clickRow['btn_401']),intval($clickRow['btn_402']),intval($clickRow['btn_501']),intval($clickRow['btn_601']),intval($clickRow['btn_602']),intval($clickRow['btn_603']),intval($clickRow['btn_604']),intval($clickRow['btn_605']),intval($clickRow['btn_701']),intval($clickRow['btn_702']),intval($clickRow['btn_801']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
