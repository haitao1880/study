...
$startday_01 = date('Y-m-d',strtotime("$tempdate -7 days"));
$endday_01 = date('Y-m-d',strtotime("$tempdate -0 days"));
...
if($dailyViewRow){
    ...
    $bind = array($startday,$endday,intval($viewRow['total_start_times']),intval($viewRow['total_pv']),$viewRow['per_view'],intval($viewRow['total_online_times']),$viewRow['per_online']);
...
if($dailyMemberRow){
    ...
    $insertSql = "INSERT INTO rhc_member_weekly(startday,endday,new_user,old_user,active_user,open_num) VALUES (?,?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($userRow['total_new_user']),$old_user,intval($userTempRow['total_active_user']),intval($userRow['total_open_num']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
if($dailyPointRow){
	...
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($pointRow['total_send_point']),intval($pointRow['total_use_point' ]),intval($pointRow['sign_point']),intval($pointRow['download_point']),intval($pointRow['buy_point']),intval($pointRow['package_point']),intval($pointRow['huoban_point']),intval($pointRow['huafei_point']),intval($pointRow['other_point']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
if($dailyClickRow){
	...
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($clickRow['btn_201']),intval($clickRow['btn_202']),intval($clickRow['btn_203']),intval($clickRow['btn_204']),intval($clickRow['btn_205']),intval($clickRow['btn_207']),intval($clickRow['btn_208']),intval($clickRow['btn_301']),intval($clickRow['btn_302']),intval($clickRow['btn_303']),intval($clickRow['btn_401']),intval($clickRow['btn_402']),intval($clickRow['btn_501']),intval($clickRow['btn_601']),intval($clickRow['btn_602']),intval($clickRow['btn_603']),intval($clickRow['btn_604']),intval($clickRow['btn_605']),intval($clickRow['btn_701']),intval($clickRow['btn_702']),intval($clickRow['btn_801']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
