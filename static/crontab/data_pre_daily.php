...
$viewSql = '
	...
	(SELECT count(id) AS total_start_times FROM rhc_game_platform WHERE cday = "'.$tempPredate.'" AND type = 1 AND pid in (1,2)) AS start_times,
    (SELECT count(id) AS total_startapp_times FROM rhc_game_platform WHERE cday = "'.$tempPredate.'" AND type = 1 AND pid = 0 ) AS startapp_times';
