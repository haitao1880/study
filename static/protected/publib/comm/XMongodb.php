...
* 创建时间:下午5:33:52
* 字符编码:UTF-8
* 版本信息:$Id: XMongodb.php 10 2014-06-13 07:03:39Z tony_ren $
* 修改日期:$LastChangedDate: 2014-06-13 15:03:39 +0800 (周五, 13 六月 2014) $
* 最后版本:$LastChangedRevision: 10 $
* 修 改 者:$LastChangedBy: tony_ren $
* 版本地址:$HeadURL: http://192.168.1.100:12000/svn/rockgame/trunk/code/gamewww/protected/publib/comm/XMongodb.php $
* 摘    要:mongodb封装
*/
...
class XMongodb {
	...
	private $CI;
	private $config_string = 'mangoDBserver';
	...
	public function __construct() {
		...
		global $G_X;
		if(empty($G_X[$this->config_string]))return false;
	 ...
	 */
	 ...
	 //edit 20130319 19:40
	 public function where_elemMatch($field="",$where=array()){
	     $this->where_init($field);
	     $this->wheres[$field]['$elemMatch'] = $where;
...
/**
	 ...
	 */
	 ...
	 public function upsert($collection = "", $data = array()) {
	 	...
	 	try {
	 		$this->db->{$collection}->update($this->wheres, array('$set' => $data), array('safe' => TRUE, 'upsert' => true ));
	 		return(TRUE);
	 	} catch(MongoCursorException $e) {
	 		show_error("Update of data into MongoDB failed: {$e->getMessage()}", 500);
	 	}
	...
	private function connection_string() {
		...
		global $G_X;
		$config=$G_X[$this->config_string];
		$this->host = trim($config['mongo_host']);
		$this->port = trim($config['mongo_port']);
		$this->user = trim($config['mongo_user']);
		$this->pass = trim($config['mongo_pass']);
		$this->dbname = trim($config['mongo_db']);
