...
if($dailyViewRow){
	...
	$bind = array();
	$bind = array(intval($viewRow['total_start_times']),intval($viewRow['total_pv']),intval($viewRow['per_view']),$total_time,$per_online,$predate);
...
if($dailyMemberRow){
    $updateSql = "UPDATE rhc_member_daily SET new_user = ?,old_user = ?,active_user = ? WHERE day = ?";
	$bind = array();
	$bind = array(intval($new_user),$old_user,intval($active_user),$predate);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_member_daily(day,new_user,old_user,active_user) VALUES (?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($predate,intval($new_user),$old_user,intval($active_user));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
$dailyPointSql = 'SELECT id FROM rhc_point_daily WHERE day = "'.$predate.'"';
...
if($dailyPointRow){
    ...
    $insertSql = "INSERT INTO rhc_point_daily(day,total_send_point,total_use_point,sign_point,download_point,buy_point,package_point,huoban_point,huafei_point,other_point) VALUES (?,?,?,?,?,?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($predate,intval($pointRow['total_send_points']),intval($pointRow['total_cost_points' ]),intval($pointRow['total_sign_points']),intval($pointRow['total_download_points']),intval($pointRow['total_buy_points']),intval($pointRow['total_package_points']),intval($pointRow['total_huoban_points']),intval($pointRow['total_huafei_points']),intval($pointRow['total_other_points']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
