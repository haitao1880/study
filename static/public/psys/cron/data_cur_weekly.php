...
$startday = date('Y-m-d',strtotime("$tempdate -7 days"));
$endday = date('Y-m-d',strtotime("$tempdate -1 days"));
$premiketime = strtotime($startday);
$curmiketime = strtotime($endday)-1;
//获取本周 平台启动次数 平台pv
$viewSql = '
...
$dailyViewSql = 'SELECT id FROM rhc_view_weekly WHERE startday = "'.$startday.'"';
...
if($dailyViewRow){
	...
	$bind = array();
	$bind = array(intval($viewRow['total_start_times']),intval($viewRow['total_pv']),intval($viewRow['per_view']),$total_time,$per_online,$startday,$endday);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_view_weekly(startday,endday,start_times,page_views,per_view,online_times,per_online) VALUES (?,?,?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($viewRow['total_start_times']),intval($viewRow['total_pv']),intval($viewRow['per_view']),$total_time,$per_online);
...
$dailyMemberSql = 'SELECT id FROM rhc_member_weekly WHERE startday = "'.$startday.'"';
...
if($dailyMemberRow){
    $updateSql = "UPDATE rhc_member_weekly SET new_user = ?,old_user = ?,active_user = ? WHERE startday = ? AND endday = ?";
	$bind = array();
	$bind = array(intval($new_user),$old_user,intval($active_user),$startday,$endday);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_member_weekly(startday,endday,new_user,old_user,active_user) VALUES (?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($new_user),$old_user,intval($active_user));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
$dailySignSql = 'SELECT id FROM rhc_sign_weekly WHERE startday = "'.$startday.'"';
...
if($dailySignRow){
    ...
    $insertSql = "INSERT INTO rhc_sign_weekly(startday,endday,sign_times,start_times) VALUES (?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($signRow['total_sign_times']),intval($signRow['total_start_times']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
$dailyPointSql = 'SELECT id FROM rhc_point_weekly WHERE startday = "'.$startday.'"';
...
if($dailyPointRow){
	...
	$bind = array();
	$bind = array(intval($pointRow['total_send_points']),intval($pointRow['total_cost_points']),intval($pointRow['total_sign_points']),intval($pointRow['total_download_points']),intval($pointRow['total_buy_points']),intval($pointRow['total_package_points']),intval($pointRow['total_huoban_points']),intval($pointRow['total_huafei_points']),intval($pointRow['total_other_points']),$startday,$endday);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_point_weekly(startday,endday,total_send_point,total_use_point,sign_point,download_point,buy_point,package_point,huoban_point,huafei_point,other_point) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($pointRow['total_send_points']),intval($pointRow['total_cost_points' ]),intval($pointRow['total_sign_points']),intval($pointRow['total_download_points']),intval($pointRow['total_buy_points']),intval($pointRow['total_package_points']),intval($pointRow['total_huoban_points']),intval($pointRow['total_huafei_points']),intval($pointRow['total_other_points']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
$dailyClickSql = 'SELECT id FROM rhc_click_weekly WHERE startday = "'.$startday.'"';
...
if($dailyClickRow){
	...
	$bind = array();
	$bind = array(intval($clickArr['201']),intval($clickArr['202']),intval($clickArr['203']),intval($clickArr['204']),intval($clickArr['205']),intval($clickArr['207']),intval($clickArr['208']),intval($clickArr['301']),intval($clickArr['302']),intval($clickArr['303']),intval($clickArr['401']),intval($clickArr['402']),intval($clickArr['501']),intval($clickArr['601']),intval($clickArr['602']),intval($clickArr['603']),intval($clickArr['604']),intval($clickArr['605']),intval($clickArr['701']),intval($clickArr['702']),intval($clickArr['801']),$startday,$endday);
	$stmt = $tongji_conn->prepare($updateSql);
	$stmt->execute($bind);
}else{
    $insertSql = "INSERT INTO rhc_click_weekly(startday,endday,btn_201,btn_202,btn_203,btn_204,btn_205,btn_207,btn_208,btn_301,btn_302,btn_303,btn_401,btn_402,btn_501,btn_601,btn_602,btn_603,btn_604,btn_605,btn_701,btn_702,btn_801) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $bind = array();
	$tongji_conn->setAttribute(PDO::ATTR_CASE, PDO::CASE_NATURAL);
    $tongji_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $bind = array($startday,$endday,intval($clickArr['201']),intval($clickArr['202']),intval($clickArr['203']),intval($clickArr['204']),intval($clickArr['205']),intval($clickArr['207']),intval($clickArr['208']),intval($clickArr['301']),intval($clickArr['302']),intval($clickArr['303']),intval($clickArr['401']),intval($clickArr['402']),intval($clickArr['501']),intval($clickArr['601']),intval($clickArr['602']),intval($clickArr['603']),intval($clickArr['604']),intval($clickArr['605']),intval($clickArr['701']),intval($clickArr['702']),intval($clickArr['801']));
    $stmt = $tongji_conn->prepare($insertSql);
	$stmt->execute($bind); 
}
$stmt = NULL;
...
$deletePkSql = 'DELETE FROM rhc_package_weekly WHERE startday = "'.$startday.'"';
