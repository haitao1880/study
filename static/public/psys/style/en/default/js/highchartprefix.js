...
function makeChart_pie(render,b_title,xv,y_title,data){
    var option = new Object();  
    option.chart = new Object();  
    option.chart.type="pie";  //"spline"图类型
    option.chart.renderTo=render; //"container"容器  
    option.chart.plotBackgroundColor=null;
    option.chart.backgroundColor="#EEEEEE";
    option.chart.plotBorderWidth=null;
    option.chart.plotShadow=false;    
    //option.chart.events={click :function(e){console.log(e)}};
    ...
    option.tooltip={crosshairs: true,shared: true};  
    option.plotOptions={pie: {allowPointSelect: true,cursor: 'pointer',dataLabels: {enabled: true}}};
    
    option.series = new Array();
    option.series[0] = new Object();
    option.series[0].name = '人数(个)';
    option.series[0].colorByPoint = true;
    option.series[0].data = new Array();
      
    for(var i=0;i<2;i++){
        option.series[0].data.push({name:data[i+1].name+"("+Number(data[i+1].data)+")", y:Number(data[i+1].data)})
    }  
    ...
    $.ajax({
         ...
         success: function(data){
            ...
            if(data.data.station != undefined){
                ...
                makeChart("bar","container2","深度比较："+data.title[0],data.xv,"人数/个",data.data,true);
                Hides = 0;
...
function output(){
    ...
    window.location.href="/access/ajaxOutputExcel?fastSearch="+$("#fastHidden").val()+"&datastatusFS="+$("input[name='datastatusFS']:checked").val()+"&dateSearch="+$("#dataSearchHidden").val()+"&oneday="+$("input[name='oneday']").val()+"&bmanyday="+$("input[name='bmanyday']").val()+"&emanyday="+$("input[name='emanyday']").val()+"&oneweek="+$("input[name='oneweek']").val()+"&bmanyweek="+$("input[name='bmanyweek']").val()+"&emanyweek="+$("input[name='emanyweek']").val()+"&onemonth="+$("input[name='onemonth']").val()+"&bmanymonth="+$("input[name='bmanymonth']").val()+"&emanymonth="+$("input[name='emanymonth']").val()+"&datastatus="+$("input[name='datastatus']:checked").val()+"&stationC="+s;
