...
class LogDo
{
	...
	{
		$sh = $this->CAT_FILE_WEB.' | grep " 200 " | grep -i "downapp/trainapp/" | cut -d " " -f 7 |cut -d "?" -f 2 | cut -d "/" -f 3,4 | cut -d "&" -f 1 | awk \'BEGIN{OFS="/"}{count[$1]++}END{for(name in count)print name,count[name]}\'';
		exec($sh,$train_down);
