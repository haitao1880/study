		...
		try {
			...
			if($ap == 1){
				...
				foreach($hourconnect as $v){
					...
					$temp += array('date'=>$this->date,'hour'=>$this->hour);
				...
				foreach($hourconnects as $v1){
					list($temp['stationid'],$temp['num']) = explode('/',$v1);
					$temp += array('date'=>$this->date,'hour'=>$this->hour);
					$this->Insert($temp,'rha_aclog_hour');
				}
				
			}
			//file_put_contents('shell.txt', $sh1.PHP_EOL,FILE_APPEND);
