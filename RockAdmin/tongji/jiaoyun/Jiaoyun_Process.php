...
class Jiaoyun_Process{
	...
	public function GetNotCount(){
		...
		$sql = "SELECT days FROM $tb WHERE t_status = 0 and car_id = $this->cid";
		$res = $this->db->FetAll($sql);
		return $res;
	}
...
$days1 = array();
$temp = array();
foreach($daystr as $k => $v){
	if (!strrpos($v['days'], ',')) {
		$temp[] = $v['days'];
	}	

	if ($k > 0) {
		$days = array_merge($temp,$days1,explode(',',$v['days']));
	}else{
		$days1 = explode(',',$v['days']);
		$days = array_merge($temp,$days1);
	}
